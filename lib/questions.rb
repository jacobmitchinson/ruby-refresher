...
def separate_array_into_even_and_odd_numbers(array)
  new_array = []
  positive = array.select { |number| number.even? }
  negative = array.select { |number| number.odd? }
  new_array << positive
  new_array << negative
end
...
def number_of_elements_that_are_palindromes(array)
  ...
  array.each do |number|
    number.reverse == number ? count += 1 : nil
  end
  return count
end
...
def get_all_letters_in_array_of_words(array)
  split =  array.map {|element| element.split('')}
  split.flatten.sort!
end
...
def get_domain_name_from_email_address(email)
  email.split('@').last.split('').take_while {|char| char != '.'}.join
end
...
def titleize_a_string(string)
  ignore = ['a', 'and', 'the']
  string = string.split(' ').map {|word| ignore.include?(word) ? word : word.capitalize! }.join(' ')
  capital = string[0].upcase!
  string.slice!(0)
  capital + string
end
...
def is_a_2015_bank_holiday?(date)
  response = HTTParty.get('https://www.gov.uk/bank-holidays')
end
